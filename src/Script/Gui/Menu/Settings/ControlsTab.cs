...
using MenuDic = System.Collections.Generic.Dictionary<string, MSG.Script.Gui.Menu.Settings.SettingsRow>;

namespace MSG.Script.Gui.Menu.Settings
{
    public class ControlsTab : HBoxContainer
    {
        ...
        {
            var menuRow = ControlVBox.AddChildAndReturn(new SettingsRow<CenterContainer>());
            menuRow.Label.Text = "Edge Scroll";
            var checkBox = menuRow.InputObject.AddChildAndReturn(new CheckBox());
            checkBox.Pressed = SettingsManager.EdgeScroll;
            checkBox.Connect(Signal.Toggled, this, nameof(OnEdgeScrollCheckButtonToggled));
            SettingsManager.OnSettingChanged += (name, val) =>
            {
                if (name == nameof(SettingsManager.EdgeScroll))
        ...
        public void LoadInputList()
        {
            ...
            foreach (var name in actionArray.SkipWhile(SettingsManager.ActionDisplayBlacklist.Contains))
        ...
        public void GenerateInputList()
        {
            ...
            {
                var row = ControlVBox.AddChildAndReturn(new SettingsRow<Button>());
                row.Label.Text = ConvertActionName(pair.Key);
                row.InputObject.Text = ConvertButtonText(pair.Value[0]);
                if (SettingsManager.ActionDescriptions.ContainsKey(pair.Key))
                    row.HintTooltip = SettingsManager.ActionDescriptions[pair.Key];
        ...
        public void HandleControlButtonPress(string actionName)
        {
            ...
            if (SettingsManager.ActionDescriptions.ContainsKey(actionName))
                RebindDialog.DescriptionLabel.Text = SettingsManager.ActionDescriptions[actionName];
        ...
        public void OnEdgeScrollCheckButtonToggled(bool pressed)
            => SettingsManager.EdgeScroll = pressed;
